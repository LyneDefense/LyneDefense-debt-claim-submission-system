...
# 功能描述: 使用 Maven 打包项目、构建 Docker 镜像、保存为 tar 文件、计算哈希并上传到服务器
...
# Docker 镜像名称和标签
IMAGE_NAME="lyneee/claim-service:latest"

# 镜像保存路径
LOCAL_SAVE_DIR="/Users/pinjiehu/docker_images"
IMAGE_FILE="${LOCAL_SAVE_DIR}/claim-service_latest.tar"
HASH_FILE="${LOCAL_SAVE_DIR}/claim-service_latest.tar.sha256"

# 服务器信息
SERVER_USER="root"
SERVER_IP="106.52.20.109"
SERVER_IMAGE_DIR="/root/docker_images"
SERVER_HASH_FILE="${SERVER_IMAGE_DIR}/claim-service_latest.tar.sha256_server_computed"
...
transfer_and_verify() {
    ...
    {
        rsync -av --progress "$HASH_FILE" "${SERVER_USER}@${SERVER_IP}:${dest_dir}/claim-service_latest.tar.sha256" 2>&1
    } | tee -a "$LOG_FILE"
    if [ ${PIPESTATUS[0]} -ne 0 ]; then
        log "ERROR" "哈希文件传输失败。"
        exit 1
    ...
    ssh "${SERVER_USER}@${SERVER_IP}" "
        shasum -a 256 ${dest_dir}/claim-service_latest.tar | awk '{print \$1}' > ${dest_dir}/claim-service_latest.tar.sha256_computed
        diff ${dest_dir}/claim-service_latest.tar.sha256 ${dest_dir}/claim-service_latest.tar.sha256_computed
    " | tee -a "$LOG_FILE"
